...
// Port of apps/web/src/relay/client.test.ts. Real sockets against a real
// (throwaway) WS server, not mocks — specifically to verify the
// reconnect-with-backoff logic in client.ts with real timing and real
// disconnect events, which a mocked connection can't exercise. RELAY_URL in
// client.ts is read from process.env.EXPO_PUBLIC_RELAY_URL once at module
// load (Expo's env var convention, vs. Vite's import.meta.env on the web
// side), so it has to be stubbed before each dynamic import.
...
describe('connectRoom', () => {
  ...
  beforeEach(() => {
    vi.resetModules()
    vi.stubEnv('EXPO_PUBLIC_RELAY_URL', `ws://127.0.0.1:${TEST_PORT}`)
  })
